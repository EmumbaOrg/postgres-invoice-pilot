...
-- Conditionally create semantic_reranking function based on model
...
BEGIN
    -- Check the deployment model and execute appropriate logic
    IF '${DEPLOY_AML_MODEL}' = 'mini' THEN
        RETURN QUERY
        WITH
        json_pairs AS (
    ...
    ELSIF '${DEPLOY_AML_MODEL}' = 'bge' THEN
        RETURN QUERY
        WITH
        json_pairs AS(
            SELECT jsonb_build_object(
    ...
    ELSE
        RAISE EXCEPTION 'Invalid DEPLOY_AML_MODEL value: %. Expected ''mini'' or ''bge''.', '${DEPLOY_AML_MODEL}';
    END IF;
