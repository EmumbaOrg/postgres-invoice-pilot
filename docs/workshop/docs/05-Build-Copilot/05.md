...
!!! danger "**Potential Errors**"
    - **Missing `.env` Error**
        Remember to add a `.env` for backend by following the steps mentioned in [section 2.6](../02-Setup/06-Setup-Dev-Env.md). Otherwise you will see this error when you run the backend api debugger.

        ![Screenshot of the VS Code Activity Bar, with the Run and Debug icon highlighted.](../img/missing-env.png)

    - **AZ login Error**

        Remember to login using `az login`. Otherwise you'll see the following error while running the backend api debugger.    

        ![Screenshot of the VS Code Activity Bar, with the Run and Debug icon highlighted.](../img/az-login-issue.png)

1. In Visual Studio Code, select the **Run and Debug** icon from the _Activity Bar_ on the top side.

    ![Screenshot of the VS Code Activity Bar, with the Run and Debug icon highlighted.](../img/5.5.1.png)

2. At the top of the **Run and Debug** menu, select the **API Debugger** option from the debug configurations dropdown list.
...
4. Once the API debugging session has started, indicated by an `Application startup complete.` message in the terminal output, followed by the URL on which Uvicorn is running, open the local API URL in a web browser (<http://127.0.0.1:8000/>).
    ...
    ![Screenshot of the VS Code terminal output for starting the API via a debug session.](../img/5.5.4.png)

5. Append `/swagger` to the URL to get the Swagger UI interactive testing page.

    ![Screenshot of the Invoice Pilot Swagger UI.](../img/invoice-pilot-api-swagger-ui.png)

6. Expand the `POST` section for the `completions/chat` endpoint under the **Completions** tag and select `Try it out`:

    ![Screenshot of the chat endpoint expanded, with the Try it out button highlighted.](../img/invoice-pilot-api-chat-try-it-out.png)
...
7. In the **Request body** section, paste the following JSON snippet to request a list of vendors, then select **Execute**.
    ...
    ![Screenshot of the request body section, with the above message highlighted in the body textbox and the Execute button highlighted.](../img/invoice-pilot-api-chat-try-it-out-execute.png)
...
8. Scroll down to the **Responses** section for the `/completions/chat` endpoint in the Swagger UI and verify you received a valid response.
    ...
    ![Screenshot of the Responses section of the Swagger UI, with the Response body populated by the response from the above query.](../img/invoice-pilot-api-chat-response.png)
