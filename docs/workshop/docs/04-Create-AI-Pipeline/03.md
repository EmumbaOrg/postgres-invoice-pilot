...
Abstractive Summarization creates summaries by generating new sentences that capture the main ideas and key information from a source text, rather than simply extracting existing sentences. Azure cognitive services supports abstractive summarization as well.

## Insert Document Summary on Database Insert

Leveraging the `azure_cognitive.summarize_abstractive` method of the `azure_ai` extension, the database scripts are able to make calls to generate a document summary on INSERT.
...
The `/sows/` HTTP POST method of the REST API contains code that inserts SOWs based on the document uploaded. The code for this is within the `src/api/app/routers/sows.py` file. Open it now in Visual Studio Code and explore the code within the `async def analyze_sow` method that contain the code to ingest SOW documents, including the portion that performs the database INSERT on the `sows` table.

You can expand the section below to see the specific section of code that performs the `azure_ai` call to generate the document summary, within the database INSERT statements.

The code implements both extractive and abstractive summarization approaches. Currently, we're using the abstractive method while the extractive implementation is commented out, allowing users to easily switch between the two techniques as needed.

???+ info "INSERT SOW with document summary generation"
...
- [Abstractive Summarization](https://learn.microsoft.com/en-gb/azure/postgresql/azure-ai/generative-ai-azure-cognitive#azure_cognitivesummarize_abstractive)
