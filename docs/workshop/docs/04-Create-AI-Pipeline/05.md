...
Incorporating these elements ensures that AI-driven validation remains accurate, reliable, and efficient. The following sections below will review the structure and elements of the Invoice and SOW validation prompts used in the current workflow.
...
???+ info "Invoice validation prompt"
    ...
    2. Check that the total amount on the invoice is sum of the corresponding individual items.
    ...
    7. Payment Terms: 'Net 30' means a 30-day payment buffer is allowed after the invoice/delivery/milestone submission date, where the number indicates the buffer period in days. This means that the payment due date is within 30 days of the submission due date.
    8. If the invoice contains notes to explain discrepancies, review them for additional context.
    9. Confirm that the invoice is legitimate and ready for payment.
    10. When comparing milestone names between the SOW and the invoice, do not require an exact match. Allow for slight differences in wording as long as the overall meaning is the same.
    ...
    An invoice does not need to contain all milestones and deliverables from the SOW. Each invoice may only bill for specific milestones and their associated deliverables. Only validate the milestones and deliverables that are actually present on the invoice against the SOW. Do not consider missing milestones or deliverables as validation failures unless they are overdue according to the SOW timeline.
...
### Structure and Key Sections for Invoice
...
???+ info "SOW validation prompt"
    ...
    1. Verify that the SOW belongs to the given vendor by making sure that the vendor name mentioned in the Project Scope of SOW matches the given vendor details given separately. Match the given vendor details with the corresponding vendor details mentioned in the SOW. If they are not same, then the SOW is invalid.
    2. The provided JSON contains different details of the SOW, the keys indicating a section of the SOW and the value of the key indicating the associated detail. If a key contains an empty value, this means that details of that section are not present. If details of any section are missing then the SOW is not valid.  
    3. Check that the total amount on the SOW is the sum of all the deliverables' amounts.
    4. Make sure that project end date is after the project start date. Also ensure that the milestone completion and payment due dates are after the project start date and before or on the specified project submission date in the SOW.
    5. Validate the deliverables for each milestone in the SOW. Ensure that the deliverale completion date is within the overall project start and completion date.
    ...
    9. Payment Terms: 'Net 30' means a 30-day payment buffer is allowed after the invoice/delivery/milestone submission date, where the number indicates the buffer period in days. This means that the payment due date is within 30 days of the submission due date.
    10. For milestones, the dates mentioned here are the due date for submission and the due date for payment, not the date on which the milestone was completed and its invoice submitted. So penalty calculation should not be performed here.
    11. For deliverables, the dates mentioned here are payment due dates which have a buffer period from the deliverable completion due date. The buffer is the number associated with 'Net'. For example 'Net 30' means a 30 day buffer is allowed for the payment of a deliverable after its completion.
    12. When comparing milestone names between the SOW and deliverables, do not require an exact match. Allow for slight differences in wording as long as the overall meaning is the same.
    13. Compare, individually, each milestone completion and payment due date and deliverable completion due date with the overall project start and end date and make sure they fall within the overall project dates. Show this comparison in your output in a text form, not in a tabular form.
...
### Structure and Key Sections for SOW
