...
???+ info "SOW validation prompt"
    ...
    1. Verify that the SOW belongs to the given vendor by making sure that the vendor name mentioend in the Project Scope of SOW matches the given vendor details given separately. Match the given vendor details with the corresponding vendor details mentioned in the SOW. If they are not same, then the SOW is invalid.
