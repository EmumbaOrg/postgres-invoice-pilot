...
Previosuly, we saw an overview of how an `embedding` column of `vector` type is added to some of our tables in the database. In this section, we’ll see the use of trigger functions that automatically generate embeddings for the `embedding` column using `azure_openai`. This ensures that embeddings are created automatically whenever new data is inserted into the tables.
